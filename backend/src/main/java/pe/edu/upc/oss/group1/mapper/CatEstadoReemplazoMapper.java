package pe.edu.upc.oss.group1.mapper;

import pe.edu.upc.oss.group1.dto.catalogo.request.CatEstadoReemplazoRequest;
import pe.edu.upc.oss.group1.dto.catalogo.response.CatEstadoReemplazoResponse;
import pe.edu.upc.oss.group1.entity.catalogo.CatEstadoReemplazo;

import java.util.List;
import java.util.stream.Collectors;

/**
 * Mapper para convertir entre entity, request DTO y response DTO.
 * Utiliza patrón de mapeo manual sin librerías externas.
 */
public class CatEstadoReemplazoMapper {

    /**
     * Convierte un Request DTO a Entity.
     */
    public static CatEstadoReemplazo toEntity(CatEstadoReemplazoRequest request) {
        CatEstadoReemplazo entity = new CatEstadoReemplazo();
        entity.setCodigo(request.getCodigo());
        entity.setNombre(request.getNombre());
        entity.setDescripcion(request.getDescripcion());
        entity.setActivo(request.getActivo());
        return entity;
    }

    /**
     * Convierte un Entity a Response DTO.
     */
    public static CatEstadoReemplazoResponse toResponse(CatEstadoReemplazo entity) {
        return CatEstadoReemplazoResponse.builder()
                .id(entity.getId())
                .codigo(entity.getCodigo())
                .nombre(entity.getNombre())
                .descripcion(entity.getDescripcion())
                .activo(entity.getActivo())
                .fechaCreacion(entity.getFechaCreacion())
                .build();
    }

    /**
     * Convierte una lista de entities a lista de response DTOs.
     */
    public static List<CatEstadoReemplazoResponse> toResponseList(List<CatEstadoReemplazo> entities) {
        return entities.stream()
                .map(CatEstadoReemplazoMapper::toResponse)
                .collect(Collectors.toList());
    }
}
